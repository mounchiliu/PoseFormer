...
if args.dataset == 'h36m':
    ...
    dataset = Human36mDataset(
        dataset_path)  # 构建human36m Dataset格式, 其中包括3D点和相机信息 (data[subject][action] = {position, camera})
...
kps_left, kps_right = list(keypoints_symmetry[0]), list(keypoints_symmetry[1])  # joints_left and joints_right
...
for subject in keypoints.keys():
    for action in keypoints[subject]:
        for cam_idx, kps in enumerate(keypoints[subject][action]):
            ...
            kps[..., :2] = normalize_screen_coordinates(kps[..., :2], w=cam['res_w'], h=cam['res_h'])  # 将kp
...
receptive_field = args.number_of_frames  # number_of_frames used for temporal transformer
...
#########################################PoseTransformer
# 2. 构建模型
model_pos_train = PoseTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=32,
                                  depth=4,
...
if args.resume or args.evaluate:  # load pretrained parameters
...
def eval_data_prepare(receptive_field, inputs_2d, inputs_3d):
    inputs_2d_p = torch.squeeze(inputs_2d)  # 1, 1765,17,2 -> 1765, 17, 12# 压缩，删除多余维度
    inputs_3d_p = inputs_3d.permute(1, 0, 2, 3)  # 1,1685,17,3 -> 1685, 1, 17, 2
    out_num = inputs_2d_p.shape[0] - receptive_field + 1  # 1765 - 81 + 1 = 1685 (81:'how many frames used as input'))
    # eval_input_2d： 1685， 81, 17, 2, 初始化全0矩阵
    eval_input_2d = torch.empty(out_num, receptive_field, inputs_2d_p.shape[1], inputs_2d_p.shape[2])
    for i in range(out_num):  # 赋值
...
if not args.evaluate:  # do not load checkpoint, -> evaluate for test, here for taining
    ...
    while epoch < args.epochs:
        ...
        N = 0
        N_semi = 0
        model_pos_train.train()  # 设置为train

        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():  # get train dara
            cameras_train = torch.from_numpy(cameras_train.astype('float32'))
            # 以某一个帧为中心，往前、往后取num_frame帧，并进行temporal transformer计算其关联
            # num_frame为用户定义的参数
            ...
            inputs_3d[:, :, 0] = 0  # 3d pose中第一个joint赋值为0
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():  # get batch data from generator
            ...
            ##### apply test-time-augmentation (following Videopose3d)
            inputs_2d_flip = inputs_2d.clone()
            inputs_2d_flip[:, :, :, 0] *= -1  # 2d kp x向量 ×-1 （augmentation）# batch, num_points, 17, 2
            # 交换张量中的相应值来翻转图像的左右关键点
            inputs_2d_flip[:, :, kps_left + kps_right, :] = inputs_2d_flip[:, :, kps_right + kps_left, :]  # left+right一共12个点
            ...
            predicted_3d_pos = model_pos(inputs_2d)  # forward, get predicted 3d Positions
            predicted_3d_pos_flip = model_pos(inputs_2d_flip)# forward, get predicted flip-3d Positions
            # 从flip中还原原数值
            predicted_3d_pos_flip[:, :, :, 0] *= -1  # 3D坐标的第一个flip （对应2D flip操作）
            predicted_3d_pos_flip[:, :, joints_left + joints_right] = predicted_3d_pos_flip[:, :,
                                                                      joints_right + joints_left]  # 同时左右点flip(对应2D flip操作)

            predicted_3d_pos = torch.mean(torch.cat((predicted_3d_pos, predicted_3d_pos_flip), dim=1), dim=1,
                                          keepdim=True)  # 计算两次计算的mean作为结果
...
if args.render:  # visualize a particular video
    print('Rendering...')
    ...
    # For testing, we use Non-batched data generator
...
else:  # load a checkpoint but do not need to visualize a video
    print('Evaluating...')  # 从dataset中读取所有test信息并整理为dict格式
    all_actions = {}
    all_actions_by_subject = {}
    for subject in subjects_test:  # loop all subjects required to be tested,S9, S11
    ...
    def fetch_actions(actions):
        ...
        for subject, action in actions:
            ...
            for i in range(len(poses_2d)):  # Iterate across cameras
                out_poses_2d.append(poses_2d[i])  # get all 2d kps

            poses_3d = dataset[subject][action]['positions_3d']
            assert len(poses_3d) == len(poses_2d), 'Camera count mismatch'
            for i in range(len(poses_3d)):  # Iterate across cameras
                out_poses_3d.append(poses_3d[i])  # get all 3D kps
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            if action_filter is not None:  # 过滤action
                found = False
                for a in action_filter:
            ...
            poses_act, poses_2d_act = fetch_actions(actions[action_key])  # (From test datasets) get all 3d kps (gt)  and 2d kps from dataset
            gen = UnchunkedGenerator(None, poses_act, poses_2d_act,
                                     pad=pad, causal_shift=causal_shift, augment=args.test_time_augmentation,
                                     kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
                                     joints_right=joints_right)  # for loading data
            e1, e2, e3, ev = evaluate(gen, action_key)  # pass through the model and get results, then calculate losses
            errors_p1.append(e1)
            errors_p2.append(e2)
            errors_p3.append(e3)
    ...
    if not args.by_subject:
        ...
        for subject in all_actions_by_subject.keys():  # break down error by subject (on evaluation)
